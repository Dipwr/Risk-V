...
// Execute 250,000 CPU cycles before pausing to check host UI/Events
constexpr uint32_t CYCLES_PER_BATCH = 250000;

// Explicitly align to 16-byte boundaries for SIMD vectorization
  ...
  if (SDL_HasClipboardText()) {
    ...
    if (text) {
      ...
      std::cout << "\n[SIM GUI] Injected " << count
                << " characters into CPU key_buffer." << std::endl;
      SDL_free(text);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  // NATIVELY allocate a 16-bit RGB565 texture. No lookup table needed!
  SDL_Texture *texture =
  ...
  while (!Verilated::gotFinish()) {
    ...
    for (uint32_t tick = 0; tick < CYCLES_PER_BATCH; ++tick) {
      ...
      if (we_mask != 0) [[unlikely]] {
        ...
        if (vram_offset < (PAGE_SIZE_BYTES * 2)) {
          ...
          vram_dirty = true; // Mark framebuffer as changed

          uint32_t offset = vram_offset & ~3u;
          uint32_t wdata = top->DwIO;

          // OPTIMIZATION: Direct pointer casting instead of memcpy
          if (we_mask == 0xF) {
          ...
          if (we_mask & 0x01) {
            ...
            vram_dirty = true; // Page flip requires a re-render
          }
        }
      }
    ...
    while (SDL_PollEvent(&event)) {
      ...
      // Mouse Button Clicks for Control Panel Buttons
        ...
        if (mx >= btn_reset.x && mx <= (btn_reset.x + btn_reset.w) &&
          ...
          // Clear keyboard queue and reset time trackers BEFORE creating new
          // model
          ...
          std::cout << "\n[SIM GUI] Hardware CPU state, PC, and context reset."
                    << std::endl;
        }
    ...
    if (now - render_last_time >= 16) {
      ...
      // 1. Render CPU Framebuffer ONLY if dirty
      if (vram_dirty) {
        ...
        // Blast the 16-bit VRAM directly into the GPU texture, no translation
        // needed!
      ...
      // Always blit the (potentially cached) texture so the window doesn't go
      // black
      ...
      SDL_RenderDrawLine(renderer, 0, SCREEN_HEIGHT, SCREEN_WIDTH,
                         SCREEN_HEIGHT);

      // Button 1: RESET CPU
      SDL_SetRenderDrawColor(renderer, 160, 40, 40, 255); // Red
      SDL_RenderFillRect(renderer, &btn_reset);
      render_text(renderer, "RESET CPU", btn_reset.x + 11, btn_reset.y + 16,
                  {255, 255, 255, 255});

      // Button 2: MODE TOGGLE
      if (paste_mode) {
        SDL_SetRenderDrawColor(renderer, 180, 110, 20,
                               255); // Amber (Paste Mode)
        SDL_RenderFillRect(renderer, &btn_mode);
        render_text(renderer, "MODE: PASTE (Ctrl+V)", btn_mode.x + 35,
                    btn_mode.y + 16, {255, 255, 255, 255});
      } else {
        SDL_SetRenderDrawColor(renderer, 30, 90, 150,
                               255); // Blue (Normal Mode)
        SDL_RenderFillRect(renderer, &btn_mode);
        render_text(renderer, "MODE: NORMAL (Direct)", btn_mode.x + 31,
                    btn_mode.y + 16, {255, 255, 255, 255});
      }
      ...
      render_text(renderer, "PASTE NOW", btn_paste.x + 11, btn_paste.y + 16,
                  {255, 255, 255, 255});
      ...
      snprintf(buf_cycles, sizeof(buf_cycles), "Cycles: %llu",
               (unsigned long long)main_time);
      snprintf(buf_queue, sizeof(buf_queue), "Buffer: %zu keys",
               key_buffer.size());

      render_text(renderer, buf_mhz, 470, SCREEN_HEIGHT + 15,
                  {220, 220, 220, 255});
      render_text(renderer, buf_fps, 470, SCREEN_HEIGHT + 40,
                  {220, 220, 220, 255});
      render_text(renderer, buf_cycles, 700, SCREEN_HEIGHT + 15,
                  {180, 220, 255, 255});
      render_text(renderer, buf_queue, 700, SCREEN_HEIGHT + 40,
                  {255, 220, 180, 255});
    ...
    if (elapsed_ms >= 1000) [[unlikely]] {
      last_mhz = static_cast<double>(cycles_this_second) /
                 (static_cast<double>(elapsed_ms) * 1000.0);
      last_fps = static_cast<double>(frames_this_second) * 1000.0 /
