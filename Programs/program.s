.equ KEY_STAT_ADDR, 0x000A0024		# Keyboard Status Register Address
...
frame_loop:
	# -------------------------------------------------------------
	# 0. CHECK KEYBOARD FOR EXIT COMMAND
	# -------------------------------------------------------------
	li	t0, KEY_STAT_ADDR
	lw	t1, 0(t0)
	andi	t1, t1, 1			# Check ready bit
	bnez	t1, exit_to_wozmon		# Key pressed -> Exit animation!
...
exit_to_wozmon:
	...
	# EXIT BACK TO WOZMON
	# -------------------------------------------------------------
	li	ra, 0x00000000			# Set return address to Wozmon entry point
	ret					# Jump back to monitor prompt!
