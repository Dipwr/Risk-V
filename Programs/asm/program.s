.equ GRAPHIC_RAM,	0x00082000		# VRAM Page 0 base address
.equ KEY_DATA_ADDR,	0x00080020		# Keyboard Data
.equ KEY_STAT_ADDR,	0x00080024		# Keyboard Status
.equ PAGE_REG_ADDR,	0x00080000		# Page Flip Register
.equ RAM_BASE_ADDR,	0x00040000

.equ COLOR_TEXT,	0x0000			# Solid Black pixel (RGB565 0x0000)
.equ COLOR_BG,		0xFFFFFFFF		# Packed double-white background (0xFFFF | 0xFFFF)
...
_start:
	...
	li	s8, COLOR_BG		# Background color (packed 32-bit word)
	li	s9, COLOR_TEXT		# Text color (16-bit halfword)
...
handle_colon:
	beqz	s2, write_mode_init
	mv	s0, s1

write_mode_init:

write_mode_loop:
	li	s1, 0			# Reset value accumulator
	li	s2, 0			# Reset digit flag for value parsing
...
skip_upper_val:
	...
	# Check for Space (' ')
	li	t0, ' '
	beq	a0, t0, handle_val_space

	# Check for Enter ('\r' / '\n')
	li	t0, 0x0D
	beq	a0, t0, handle_val_enter
	li	t0, 0x0A
	beq	a0, t0, handle_val_enter

	# Parse Hex Digits ('0'-'9', 'A'-'F')
	li	t0, '0'
...
shift_val_nibble:
	...
	slli	s1, s1, 4
	or	s1, s1, t0
	li	s2, 1			# Set digit flag: valid value hex digit parsed
	j	parse_val_loop

handle_val_space:
	beqz	s2, parse_val_loop	# Ignore space if no digits typed yet
	sw	s1, 0(s0)		# Write 32-bit word to RAM
	addi	s0, s0, 4		# Advance destination pointer
	j	write_mode_loop

handle_val_enter:
	beqz	s2, char_newline_then_prompt # If Enter pressed with no value, exit write mode
	sw	s1, 0(s0)		# Write final word to RAM
	addi	s0, s0, 4		# Advance destination pointer
	j	char_newline_then_prompt
...
# =====================================================================
# GRAPHIC FONT ENGINE & SUBROUTINES (320x200 16-Bit Resolution)
# =====================================================================

# Clears the entire Graphic RAM (320x200 = 64,000 pixels = 128,000 bytes).
clear_screen_sub:
	mv	t0, s5			# t0 = GRAPHIC_RAM
	li	t1, 4000		# 128,000 bytes / 32 bytes per iteration
clear_loop:
	sw	s8, 0(t0)
	sw	s8, 4(t0)
	sw	s8, 8(t0)
...
draw_char:
	...
	add	t4, t4, t0		# + BaseX (Pixel index)
	slli	t4, t4, 1		# -> byte offset (* 2 bytes/pixel)
