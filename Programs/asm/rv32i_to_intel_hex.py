...
def get_riscv_toolchain():
	"""Finds the available RISC-V assembler and objcopy executables across platforms."""
	prefixes = [
		"riscv64-elf-",          # macOS Homebrew (riscv64-elf-binutils)
		"riscv64-unknown-elf-",  # macOS Homebrew (riscv-gnu-toolchain)
		"riscv64-linux-gnu-",    # Linux / Fedora / Debian
	]

	for prefix in prefixes:
		as_bin = f"{prefix}as"
		objcopy_bin = f"{prefix}objcopy"
		if shutil.which(as_bin) and shutil.which(objcopy_bin):
			return as_bin, objcopy_bin

	return None, None


def assemble_to_binary(asm_file, bin_file):
	"""Compiles RISC-V assembly into raw machine code binary using available toolchain."""
	as_bin, objcopy_bin = get_riscv_toolchain()

	if not as_bin:
		print("Error: RISC-V toolchain not found in PATH!")
		print("Make sure 'riscv64-elf-binutils' (macOS) or 'binutils-riscv64-linux-gnu' (Linux) is installed.")
		sys.exit(1)
