...
/* 32-Bit Fast Rectangular Desktop Fill */
...
void ctk_draw_widget(struct ctk_widget *widget, unsigned char focus,
  ...
  } else if (widget->type == CTK_WIDGET_ICON) {
    struct ctk_icon *icon = (struct ctk_icon *)widget;
    uint16_t fg = focus ? current_theme.text_hi : current_theme.text_fg;
    uint16_t bg = focus ? current_theme.title_act : current_theme.win_bg;
    int cur_y = y;

#if CTK_CONF_ICON_TEXTMAPS
    if (icon->textmap != NULL) {
      for (int row = 0; row < 3; row++) {
        if (cur_y >= clipy1 && cur_y < clipy2) {
          for (int col = 0; col < 3; col++) {
            draw_char(x + col, cur_y, icon->textmap[row * 3 + col], fg, bg);
          }
        }
        cur_y++;
      }
...
#endif
    if (icon->title != NULL && cur_y >= clipy1 && cur_y < clipy2) {
      draw_str(x, cur_y, icon->title, fg, bg);
    }
  }
}
...
void ctk_draw_menus(struct ctk_menus *menus) {
  ...
  /* 3. Draw Desktop Menu on the far right */
