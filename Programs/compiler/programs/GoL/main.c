/* ============================================================================
 * RV32IM BARE-METAL CONWAY'S GAME OF LIFE
...
/* SplitMix32 Bit Mixer (Now utilizing native hardware multiplication!) */
static uint32_t proper_hash(int x, int y, uint32_t seed) {
  uint32_t h = (uint32_t)(x * 0x1F3D) + (uint32_t)(y * 0x3079) + seed;
  h ^= h >> 16;
  h = h * 0x85ebca6b;
  h ^= h >> 13;
  h = h * 0xc2b2ae35;
...
void seed_grid_patterns(void) {
  ...
  set_cell(gx + 22, gy + 1, 1);
  set_cell(gx + 24, gy + 1, 1);
  set_cell(gx + 12, gy + 2, 1);
  set_cell(gx + 13, gy + 2, 1);
  set_cell(gx + 20, gy + 2, 1);
  set_cell(gx + 21, gy + 2, 1);
  set_cell(gx + 34, gy + 2, 1);
  set_cell(gx + 35, gy + 2, 1);
  set_cell(gx + 11, gy + 3, 1);
  set_cell(gx + 15, gy + 3, 1);
  set_cell(gx + 20, gy + 3, 1);
  set_cell(gx + 21, gy + 3, 1);
  set_cell(gx + 34, gy + 3, 1);
  set_cell(gx + 35, gy + 3, 1);
  set_cell(gx + 0, gy + 4, 1);
  set_cell(gx + 1, gy + 4, 1);
  set_cell(gx + 10, gy + 4, 1);
  set_cell(gx + 16, gy + 4, 1);
  set_cell(gx + 20, gy + 4, 1);
  set_cell(gx + 21, gy + 4, 1);
  set_cell(gx + 0, gy + 5, 1);
  set_cell(gx + 1, gy + 5, 1);
  set_cell(gx + 10, gy + 5, 1);
  set_cell(gx + 14, gy + 5, 1);
  set_cell(gx + 16, gy + 5, 1);
  set_cell(gx + 17, gy + 5, 1);
  set_cell(gx + 22, gy + 5, 1);
  set_cell(gx + 24, gy + 5, 1);
  set_cell(gx + 10, gy + 6, 1);
  set_cell(gx + 16, gy + 6, 1);
  set_cell(gx + 24, gy + 6, 1);
  set_cell(gx + 11, gy + 7, 1);
  set_cell(gx + 15, gy + 7, 1);
  set_cell(gx + 12, gy + 8, 1);
  set_cell(gx + 13, gy + 8, 1);

  // Acorn Methuselah (Center)
  int ax = 28, ay = 25;
  set_cell(ax + 1, ay + 0, 1);
  set_cell(ax + 3, ay + 1, 1);
  set_cell(ax + 0, ay + 2, 1);
  set_cell(ax + 1, ay + 2, 1);
  set_cell(ax + 4, ay + 2, 1);
  set_cell(ax + 5, ay + 2, 1);
  set_cell(ax + 6, ay + 2, 1);

  // Pulsar Oscillator (Right Side)
  int px = 46, py = 12;
  for (int i = 0; i < 3; i++) {
    set_cell(px + 2 + i, py + 0, 1);
    set_cell(px + 8 + i, py + 0, 1);
    set_cell(px + 2 + i, py + 5, 1);
    set_cell(px + 8 + i, py + 5, 1);
    set_cell(px + 2 + i, py + 7, 1);
    set_cell(px + 8 + i, py + 7, 1);
    set_cell(px + 2 + i, py + 12, 1);
    set_cell(px + 8 + i, py + 12, 1);
  }
  for (int j = 0; j < 3; j++) {
    set_cell(px + 0, py + 2 + j, 1);
    set_cell(px + 5, py + 2 + j, 1);
    set_cell(px + 7, py + 2 + j, 1);
    set_cell(px + 12, py + 2 + j, 1);
    set_cell(px + 0, py + 8 + j, 1);
    set_cell(px + 5, py + 8 + j, 1);
    set_cell(px + 7, py + 8 + j, 1);
    set_cell(px + 12, py + 8 + j, 1);
  }

  // Lightweight Spaceship (Bottom Left)
  int lx = 3, ly = 32;
  set_cell(lx + 1, ly + 0, 1);
  set_cell(lx + 4, ly + 0, 1);
  set_cell(lx + 0, ly + 1, 1);
  set_cell(lx + 0, ly + 2, 1);
  set_cell(lx + 4, ly + 2, 1);
  set_cell(lx + 0, ly + 3, 1);
  set_cell(lx + 1, ly + 3, 1);
  set_cell(lx + 2, ly + 3, 1);
  set_cell(lx + 3, ly + 3, 1);
}
...
/* --- Main Program --- */
int main(void) {
  print_string("[RV32IM Core] Conway's Game of Life (16-bit RGB565)\n");
  print_string("Controls: [P/Space] Pause | [N] Step | [R] Random Seed | [S] "
               "Preset | [Q/ESC] Quit\n");
