/* ============================================================================
 * RV32I BARE-METAL MANDELBROT FRACTAL RENDERER (RGB565 & SSAA)
 * ============================================================================
 */
...
// Pre-computed 16-bit RGB565 Palette LUT
...
// ----------------------------------------------------
// Palette Generation (RGB565 Gradient)
// ----------------------------------------------------
void init_palette(void) {
	for (int i = 0; i < MAX_ITER; i++) {
		...
		if (i < 8) {
			// Phase 1: Deep Blue -> Cyan
			r = 0;
			g = (i * 63) / 7;
			b = 31;
		} else if (i < 15) {
			// Phase 2: Cyan -> Warm Yellow
			int idx = i - 8;
			r = (idx * 31) / 6;
			g = 63;
			b = 31 - (idx * 31) / 6;
		} else {
			// Phase 3: Yellow -> Bright Orange/Red
			int idx = i - 15;
			r = 31;
			g = 63 - (idx * 40) / 5;
			b = 0;
		...
		palette_lut[i] = (uint16_t)(((r & 0x1F) << 11) | ((g & 0x3F) << 5) | (b & 0x1F));
	}
	// Solid Black for interior set
	palette_lut[MAX_ITER] = 0x0000;
}
...
// Render loop with 2x2 SSAA (RGB565 Re-constructed)
void render_mandelbrot(int page, int center_x, int center_y, int scale) {
	...
	for (int py = 0; py < SCREEN_HEIGHT; py++) {
		...
		for (int px = 0; px < SCREEN_WIDTH; px++) {
			...
			// Average (divide by 4 via >> 2) and pack back to 16-bit RGB565
