...
/* 32-bit Word VRAM Array Pointer (1 pixel per 32-bit word) */
...
/* --- Color Palette (RGB555 format stored in low 16 bits) --- */
#define MAKE_RGB(r, g, b) (0x8000 | (((r) & 0x1F) << 10) | (((g) & 0x1F) << 5) | ((b) & 0x1F))
#define COLOR_BG     0x8000                  // Black background
#define COLOR_BORDER MAKE_RGB(15, 15, 31)   // Bright bluish-gray frame
#define COLOR_ALIVE  MAKE_RGB(0, 31, 10)    // High-visibility Lime Green
#define COLOR_DEAD   MAKE_RGB(2, 4, 8)      // Dark slate background cell
...
/* Native 32-bit Word Grid Arrays (Forces lw/sw instructions exclusively) */
static uint32_t grid[GRID_SIZE];
static uint32_t next_grid[GRID_SIZE];
static uint32_t current_seed = 0x1337BEEF;

/* SplitMix32 / MurmurHash3 Bit Mixer */
...
static inline void set_cell(int x, int y, uint32_t val) {
...
/* --- Display Routines --- */
void clear_page(int page, uint32_t color) {
...
void draw_frame_border(int page) {
	uint32_t page_offset = page * PAGE_WORDS;
...
void render_grid(int page) {
	...
	for (int y = 0; y < GRID_H; y++) {
		...
		for (int x = 0; x < GRID_W; x++) {
			...
			uint32_t color = grid[row_idx + x] ? COLOR_ALIVE : COLOR_DEAD;
...
/* --- Main Program --- */
int main(void) {
	print_string("[RV32I Core] Game of Life (Word-Aligned Memory Fix).\n");
	...
	while (running) {
		while (*KEY_STAT != 0) {
			...
			if (key == 'p' || key == 'P' || key == ' ') {
				...
				render_grid(0);
				render_grid(1);
				print_string("[New Random Universe Generated & Paused]\n");
			} else if (key == 's' || key == 'S') {
				seed_grid_patterns();
				paused = 1;
				step_once = 0;
